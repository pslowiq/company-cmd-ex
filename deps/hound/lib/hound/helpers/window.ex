defmodule Hound.Helpers.Window do
  ...
  @spec get_window_position(String.t) :: tuple
  def get_window_position(window_handle) do
    session_id = Hound.current_session_id
    position = make_req(:get, "session/#{session_id}/window/#{window_handle}/position")
    {Map.get(position, "x"), Map.get(position, "y")}
  end

  @doc "Set window position"
  @spec set_window_position(String.t, integer, integer) :: :ok
  def set_window_position(window_handle, x, y) do
    session_id = Hound.current_session_id
    make_req(:post, "session/#{session_id}/window/#{window_handle}/position", %{x: x, y: y})
  end
